...
02/06/2024 Notes:
-  How can we adapt our model to look at trading under environmental cutoffs rather than  drought cutoffs?, such as in Salt Lake Basin
- keep farmer value constant while minimizing water use
- how do we figure out how to compensate farmers for cutting them off? See reetwika paper 3
...
num_agents = int(200) #  determining the number of agents?
dr_num = int(100) # num_agents-dr_num is num of buyers. if 50, agents 1-50 are sellers, and 51-500 are buyers #we could instead create a distribution that follows a series of climatic shocks, calibrated from real data.
