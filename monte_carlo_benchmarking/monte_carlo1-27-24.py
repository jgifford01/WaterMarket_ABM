...
globaal_array = []
count = 0
percdntsge_trade_for_all_dr_num_in_mc_sim = []
time_compexity_timer = []
gainz_from_trade_for_all_dr_num_in_mc_sim = []
num_agents_array_test = np.arange(150,450,10) #  determining the number of agents?
random.seed(42) #random seed  
## num_agents-dr_num is num of buyers. if 50, agents 1-50 are sellers, and 51-500 are buyers #we could instead create a distribution that follows a series of climatic shocks, calibrated from real data.
global_begin = time.time()  
iter_range_monte_carlo = 1

for kl in range(len(num_agents_array_test)):
    for _ in range(iter_range_monte_carlo):
        num_simulations = len(num_agents_array_test)*iter_range_monte_carlo
        count += 1
        ...
        num_agents = int(num_agents_array_test[kl]) #  determining the number of agents?
        
        dr_num = int(num_agents_array_test[kl]*0.15) # num_agents-dr_num is num of buyers. if 50, agents 1-50 are sellers, and 51-500 are buyers #we could instead create a distribution that follows a series of climatic shocks, calibrated from real data.
        ...
        if sum(qq_gft) != 0:  # else, repeat loop
            ...
            #print("dr_num",dr_num)
            #print("number agents",num_of_agents)
        ...
        global_time_diff = time.time()-global_begin


        #print('##############################################################')
        #print('Done with simulation number',count, 'in',end_time, 'seconds ')
        #estimated_time_remaining = (global_time_diff/count)*(num_simulations-count)
        #print('Estimated time remaining is',estimated_time_remaining/60,'minutes')
        #print('##############################################################')

        percdntsge_trade_for_all_dr_num_in_mc_sim = np.append(percdntsge_trade_for_all_dr_num_in_mc_sim,trade_percent)
        gainz_from_trade_for_all_dr_num_in_mc_sim = np.append(gainz_from_trade_for_all_dr_num_in_mc_sim,sum(qq_gft))

        array_loop = np.array([num_agents, np.mean(gainz_from_trade_for_all_dr_num_in_mc_sim), np.mean(percdntsge_trade_for_all_dr_num_in_mc_sim),end_time])
        globaal_array.append(array_loop)
        dfz = pd.DataFrame(globaal_array)
# Get the current date and time
current_time = datetime.now().strftime("%Y-%m-%d")
output_directory = 'monte_carlo_benchmarking/outputs'
os.makedirs(output_directory, exist_ok=True)

# Create the filename with the current time and subdirectory
file_path = os.path.join(output_directory, f'output_file_gft_pcttrade_{current_time}.csv')

# Convert the list to a Pandas DataFrame
df = pd.DataFrame(globaal_array, columns=['Num Agents', 'Mean Gainz from Trade', 'Mean Percentage Trade', 'Run Time'])
