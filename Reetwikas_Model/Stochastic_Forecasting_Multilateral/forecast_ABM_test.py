	...
	for jjj in range(len(dr_num)):
		...
		for ii in range(1):
			...
			if sum(qq_gft) != 0:
				...
				df.to_csv('all_agents_data_pos_try29_dr_no.csv',index=False)
...
df=pd.DataFrame(my_dict)
df.to_csv('try_29_multilateral_baseline_avg_data.csv',index=False) #,columns=['Percentage_of_Sellers','Percentage_Value_Increased','Percentage_of_Agents_Trading','Water_held_in_Water_Rights'])
